...
use super::{
    ...
    LockingStatement, MatchRecognizePattern, Measure, Merge, MergeAction, MergeClause,
...
impl Spanned for LockingStatement {
    fn span(&self) -> Span {
        union_spans(
            self.locking
                .iter()
                .map(|c| c.span())
                .chain(self.statement.iter().map(|s| s.span())),
        )
    }
}
